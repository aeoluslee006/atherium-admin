...
export default function MyPageShopNewPage() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      const { data } = await supabase.auth.getSession();
      if (!data.session) {
        router.replace('/login?next=/mypage/shop/new');
        return;
      }
      try {
    ...
    <div className="container">
      <div className="row-between">
        <div>
          ...
          <p className="hint-text">
            ...
            {sponsor.plan_tier === 'extended' ? ' (프로)' : ' (일반)'}
          </p>
        </div>
        <Link href="/mypage/shop" className="btn btn-outline">
          가게 관리
      ...
      {atLimit || needsUpgrade ? (
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          <p className="hint-text" style={{ marginTop: 8 }}>
            프로 셀러 또는 상품 10개 추가(+ $8/월)로 한도를 늘릴 수 있습니다.
          </p>
          <Link href="/mypage/shop" className="btn" style={{ marginTop: 12, display: 'inline-flex' }}>
            요금제 관리
          </Link>
        </div>
      ) : null}
      ...
      <form className="card form-card" onSubmit={handleSubmit}>
        ...
        <input
          ...
          disabled={atLimit}
        />
        <p className="hint-text">여러 장은 쉼표로 구분해 입력할 수 있습니다.</p>
